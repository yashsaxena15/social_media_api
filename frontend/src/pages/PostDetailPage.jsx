...
const PostDetailPage = () => {
  ...
  return (
    <div>
      ...
      <button
        onClick={() => navigate(-1)}
        className="flex items-center gap-2 text-gray-600 dark:text-slate-400 hover:text-brand-blue dark:hover:text-brand-teal mb-6 transition-colors"
      >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-lg shadow-sm border border-gray-200 dark:border-slate-800 overflow-hidden transition-colors">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-slate-800">
          <Link to={`/profile/${post.username}`} className="flex items-center gap-3">
            {post.profile_image ? (
              ...
              <div className="w-11 h-11 rounded-full bg-brand-purple/10 dark:bg-brand-purple/20 flex items-center justify-center text-brand-purple dark:text-brand-teal font-bold text-lg">
                {post.username.charAt(0).toUpperCase()}
              </div>
            )}
            <div>
              <h3 className="font-bold text-gray-900 dark:text-slate-100 hover:underline">{post.username}</h3>
        ...
        {post.caption && (
          <div className="p-4">
            <p className="text-gray-800 dark:text-slate-200 whitespace-pre-wrap text-base leading-relaxed">
      ...
      {confirmModal && (
        <ConfirmModal
          ...
          onConfirm={() =>
            confirmModal.type === 'post'
              ? handleDeletePost()
              : handleCommentDelete(confirmModal.commentId)
          }
